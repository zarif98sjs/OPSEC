...
class MStream:
    ...
    def getDocuments(self, dataDir):
        self.documentSet = DocumentSet(dataDir / self.dataset, self.wordToIdMap, self.wordList)
    ...
    def runMStream(self, sampleNo, outputPath):
        ...
        model = Model(self.K, self.MaxBatch, self.V, self.iterNum, self.alpha, self.beta, self.dataset,
                      ParametersStr, sampleNo, self.wordsInTopicNum, self.timefil)
